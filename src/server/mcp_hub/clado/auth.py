...
def get_clado_api_key() -> str:
    """
    Retrieves the Clado API key from environment variables.
    """
    api_key = os.getenv("CLADO_API_KEY")
    if not api_key or "your_clado_api_key" in api_key:
        raise ToolError(
            "Clado API key is not configured. "
            "Please set CLADO_API_KEY in your server's .env file."
        )
    return api_key