...
interface TaskDetailsProps {
	...
	onPauseTask?: (taskId: string) => void // Make optional if not always provided
}
...
const TaskDetails: FC<TaskDetailsProps> = ({
	...
	const handleFieldChange = (field: keyof Task, value: any) =>
		setEditableTask((prev) => ({ ...prev, [field]: value } as Task))
	const handleScheduleChange = (newSchedule: any) =>
		setEditableTask((prev) => ({ ...prev, schedule: newSchedule } as Task))
	const handleAddStep = () =>
	...
	const handleSaveEdit = () => {
		if (editableTask) onSave(editableTask)
		setIsEditing(false)
	}
						...
						{isEditing ? (
							...
							<div className="flex flex-col gap-4">
								...
								<div className="flex flex-col sm:flex-row gap-2 w-full">
									...
									{task.task_type === "long_form" &&
										task.orchestrator_state
											?.current_state === "ACTIVE" && onPauseTask && (
											<Button
												onClick={() =>
