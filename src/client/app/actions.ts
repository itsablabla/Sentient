...
interface PwaSubscription {
	endpoint: string;
	keys: { p256dh: string; auth: string };
}

export async function subscribeUser(subscription: PwaSubscription) {
...
export async function sendNotificationToCurrentUser(payload: any) {
	const session = await auth0.getSession()
	if (!session?.user) {
		throw new Error("Not authenticated")
	}
	...
	try {
		...
		const subscriptions: PwaSubscription[] | undefined = userProfile?.userData?.pwa_subscriptions
		...
		let successCount = 0
		const promises = subscriptions.map((subscription: PwaSubscription) => {
			return webpush
