...
function TasksPageContent() {
	...
	const demoWorkflow: Task | null = useMemo(() => {
	...
	const demoTask: Task | null = useMemo(() => {
	...
	const tasksWithDemo: Task[] = useMemo(() => {
		...
		return [demoTask, demoWorkflow, ...allTasks].filter(Boolean) as Task[]
	...
	const handleAnswerClarifications = (taskId: string, answers: any[]) => {
	...
	const handleSendChatMessage = (taskId: string, message: string) => {
		sendChatMessageMutation.mutate({ taskId, message })
	}

	const handleSelectItem = (item: Task) => {
