...
import { ChatMessage, UserProfile, Integration } from "@/types"
...
export default function ChatPage() {
	...
	const { data: userDetails } = useQuery<UserProfile>({
	...
	const handleVoiceEvent = useCallback(
		(event: any) => {
			const addMessageToCache = (message: ChatMessage) => {
				queryClient.setQueryData(["chatHistory"], (oldData: any) => {
					if (!oldData)
						return {
	...
	const handleStartVoice = async () => {
		...
		try {
			...
			if ((error as ErrorWithStatus).status === 429) {
				...
				toast.error(
					`Failed to connect: ${(error as Error).message || "Unknown error"}`
				)
			}
	...
	const handleStopVoice = () => {
		...
		if (voiceModeStartTimeRef.current !== null) {
