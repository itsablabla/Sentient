import { Auth0Client, Session } from "@auth0/nextjs-auth0/server"

const isSelfHost = process.env.NEXT_PUBLIC_ENVIRONMENT === "selfhost"

// Initialize the Auth0 client
export const auth0 = isSelfHost
	? null as any // To satisfy TypeScript when it's not used
	: new Auth0Client({
			...
			async beforeSessionSaved(session: Session, idToken: any) {
				return session
		},
		})

export async function getBackendAuthHeader(): Promise<HeadersInit | null> {
	if (isSelfHost) {
